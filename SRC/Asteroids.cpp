...
void Asteroids::Start()
{
	...
	Animation* item_anim = AnimationManager::GetInstance().CreateAnimationFromFile("item", 128, 128, 128, 128, "item_fs.png");
	Animation* enemyspaceship_anim = AnimationManager::GetInstance().CreateAnimationFromFile("enemySpaceship2", 128, 128, 128, 128, "enemyspaceship_fs2.png");
...
void Asteroids::OnSpecialKeyPressed(int key, int x, int y)
{
	...
	case GLUT_KEY_LEFT: mSpaceship->Rotate(190); break;
	// If right arrow key is pressed start rotating clockwise
	case GLUT_KEY_RIGHT: mSpaceship->Rotate(-190); break;
...
void Asteroids::OnObjectRemoved(GameWorld* world, shared_ptr<GameObject> object)
{
	if (object->GetType() == GameObjectType("Asteroid"))
	{
		...
		if (rand() % 12 < 1) {
			mGameWorld->AddObject(CreateItem());
		}
...
shared_ptr<GameObject> Asteroids::CreateItem() // double bullets
{
	shared_ptr<GameObject> item = make_shared<Item>("item");
	item->SetBoundingShape(make_shared<BoundingSphere>(item->GetThisPtr(), 4.0f));
	Animation* anim_ptr = AnimationManager::GetInstance().GetAnimationByName("item");
	shared_ptr<Sprite> live_sprite =
		make_shared<Sprite>(anim_ptr->GetWidth(), anim_ptr->GetHeight(), anim_ptr);
	item->SetSprite(live_sprite);
	item->SetScale(0.06f);

	((Item*)item.get())->AddListener((IItemListener*)mSpaceship.get());
...
shared_ptr<GameObject> Asteroids::CreateEnemySpaceship()
{
	...
	Animation* anim_ptr = AnimationManager::GetInstance().GetAnimationByName("enemySpaceship2");
